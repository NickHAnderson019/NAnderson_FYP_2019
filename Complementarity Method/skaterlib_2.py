...
def print_results(m,N, ground_constraints, contact_constraints, joint_constraints, joints, Fs, bodies):
        ...
        for j in joints:
            for body in bodies:
                for jc in joint_constraints:
                    penalty_sum_joint += m.joint_penalty[n,body,j,jc].value 
        force_sum  += m.F_a[n,'mono1'].value**2 + m.F_a[n,'mono2'].value**2
        torque_sum += m.tau_a[n,'mono1'].value**2 + m.tau_a[n,'mono2'].value**2
...
def get_max_values(m, N, ground_constraints, contact_constraints, joint_constraints, joints, WDOFs, DOF_b, Fs, signs, GRFs, bodies):
    maxForce = 0
    maxGRF = 0
    maxGRFbf = 0
    maxF_a = 0
    ...
    for i in range(1,N+1):
        ...
        for body in bodies:
            if body!='board':
                if abs(m.F_a[i,body].value)>maxF_a:
                    maxF_a = abs(m.F_a[i,body].value)
...
def make_realtime_animation(m, N, hm):
    ...
    q_xb2_ar = np.zeros(N)
    q_yb2_ar = np.zeros(N)
    q_thb2_ar = np.zeros(N)
    q_thl2_ar = np.zeros(N)
    q_r2_ar = np.zeros(N)
    ...
    for i in range (1,N+1):
        ...
        q_xb2_ar[i-1]    = m.q[i,'xb2'].value
        q_yb2_ar[i-1]    = m.q[i,'yb2'].value
        q_thb2_ar[i-1]  = m.q[i,'thb2'].value
        q_thl2_ar[i-1]  = m.q[i,'thl2'].value
        q_r2_ar[i-1]  = m.q[i,'r2'].value
    ...
    f_xb2_interp = interp1d(t_ar, q_xb2_ar, kind='cubic')
    f_yb2_interp = interp1d(t_ar, q_yb2_ar, kind='cubic')
    f_thb2_interp = interp1d(t_ar, q_thb2_ar, kind='cubic')
    f_thl2_interp = interp1d(t_ar, q_thl2_ar, kind='cubic')
    f_r2_interp = interp1d(t_ar, q_r2_ar, kind='cubic')
    ...
    q_xb2_interp = f_xb2_interp(t_d)
    q_yb2_interp = f_yb2_interp(t_d)
    q_thb2_interp = f_thb2_interp(t_d)
    q_thl2_interp = f_thl2_interp(t_d)
    q_r2_interp = f_r2_interp(t_d)
    ...
    def plot_board(i,m,ax): #update function for animation
        ...
        #plot monopod 2
        #plot body
        body_xb = q_xb2_interp[i] - 0.5*m.len[('body',1)]*cos(q_thb2_interp[i])
        body_yb = q_yb2_interp[i] - 0.5*m.len[('body',1)]*sin(q_thb2_interp[i])
        body_xf = q_xb2_interp[i] + 0.5*m.len[('body',1)]*cos(q_thb2_interp[i])
        body_yf = q_yb2_interp[i] + 0.5*m.len[('body',1)]*sin(q_thb2_interp[i])  
        ax.plot([body_xb,body_xf],[body_yb,body_yf],color='k')

        #plot leg 1
        leg1_xt = q_xb2_interp[i]
        leg1_yt = q_yb2_interp[i]
        leg1_xb = q_xb2_interp[i] + m.len[('leg',1)]*sin(q_thb2_interp[i]+q_thl2_interp[i])
        leg1_yb = q_yb2_interp[i] - m.len[('leg',1)]*cos(q_thb2_interp[i]+q_thl2_interp[i])
        ax.plot([leg1_xt,leg1_xb],[leg1_yt,leg1_yb],color='k')

        #plot leg 2
        Lt = 0.5*m.len[('leg',1)] + q_r2_interp[i] - 0.5*m.len[('leg',2)]
        Lb = 0.5*m.len[('leg',1)] + q_r2_interp[i] + 0.5*m.len[('leg',2)]
        leg2_xt = q_xb2_interp[i] + Lt*sin(q_thb2_interp[i]+q_thl2_interp[i])
        leg2_yt = q_yb2_interp[i] - Lt*cos(q_thb2_interp[i]+q_thl2_interp[i])
        leg2_xb = q_xb2_interp[i] + Lb*sin(q_thb2_interp[i]+q_thl2_interp[i])
        leg2_yb = q_yb2_interp[i] - Lb*cos(q_thb2_interp[i]+q_thl2_interp[i])
        ax.plot([leg2_xt,leg2_xb],[leg2_yt,leg2_yb],color='k')
        ax.plot(leg2_xb,leg2_yb,color='gray',marker = '.')
