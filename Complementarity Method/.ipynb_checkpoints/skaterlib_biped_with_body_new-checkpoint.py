...
def define_EOMs(m,BW,lambEOMx_board,lambEOMy_board,lambEOMth_board,lambEOMxp_biped,lambEOMyp_biped,lambEOMthp_biped,
                lambEOMthb_biped,lambEOMthll_biped,lambEOMthlr_biped,lambEOMrl_biped,lambEOMrr_biped):
    # EOMS OF SKATEBOARD -------------------------------------------------------------------------------------------------------
    def EOMx_board(m,n):
        ...
        GBW_in = BW*(m.GRF[n,'BW']+m.GRF_s[n,'BW'])
        GFW_in = BW*(m.GRF[n,'FW']+m.GRF_s[n,'FW'])
    ...
    def EOMxp_biped(m,n):
        ...
        # list the model versions of all quantities in the same order as sym_list
        var_list = [m.g,m.m[('pelvis',1)], m.m[('body',1)],m.m[('left_leg',1)],m.m[('left_leg',2)],m.m[('right_leg',1)],m.m[('right_leg',2)],
                    m.len[('pelvis',1)],m.len[('body',1)],m.len[('left_leg',1)],m.len[('left_leg',2)],m.len[('right_leg',1)],m.len[('right_leg',2)],
                    m.In[('pelvis',1)],m.In[('body',1)],m.In[('left_leg',1)],m.In[('left_leg',2)],m.In[('right_leg',1)],m.In[('right_leg',2)],
                    m.q[n,'xp'],m.q[n,'yp'],m.q[n,'thp'],m.q[n,'thb'],m.q[n,'thl_l'],m.q[n,'r_l'],m.q[n,'thl_r'],m.q[n,'r_r'],
                    m.dq[n,'xp'],m.dq[n,'yp'],m.dq[n,'thp'],m.dq[n,'thb'],m.dq[n,'thl_l'],m.dq[n,'r_l'],m.dq[n,'thl_r'],m.dq[n,'r_r'],
                    m.ddq[n,'xp'],m.ddq[n,'yp'],m.ddq[n,'thp'],m.ddq[n,'thb'],m.ddq[n,'thl_l'],m.ddq[n,'r_l'],m.ddq[n,'thl_r'],m.ddq[n,'r_r'],
                    F_a_l_in, F_a_r_in, tau_a_b_in, tau_a_l_in, tau_a_r_in, GRFx_l_in, GRFy_l_in, GRFx_r_in, GRFy_r_in]
...
def print_results(m, N, hm, ground_constraints, contact_constraints, joint_constraints, ground_func_constraints, joints, Fs, legs):
    penalty_sum_ground = 0
    penalty_sum_ground_func = 0
    ...
    for n in range(1,N+1):
        ...
        for gfc in ground_func_constraints:
            penalty_sum_ground_func += m.ground_func_penalty[n,gfc].value
        ...
        torque_sum += (m.tau_a[n,'BL'].value**2 + m.tau_a[n,'FL'].value**2 + m.tau_b[n].value**2)*hm*m.h[n].value
                
    print("--------------------------------")
    print("GROUND:  ", penalty_sum_ground)
    print("GROUND2: ", penalty_sum_ground_func)
...
def get_max_values(m, N, ground_constraints, contact_constraints, joint_constraints, joints, WDOFs, DOF_b, Fs, signs, GRFs, legs):
    ...
    for i in range(1,N+1):
        ...
        for grf in GRFs:
            ...
            if m.GRF_s[i,grf].value is not None: 
                if m.GRF_s[i,grf].value>maxGRF:
                     maxGRF = m.GRF_s[i,grf].value
        for grf in WDOFs:
            for fs in Fs:
...
def make_animation(m, Ns, scenario, X_step, Y_step, A_step, Y_obs, maxForce, maxGRF, maxGRFbf, maxF_a, title):
    ...
    if scenario == "OU": 
        ...
        GNDy = Y_step*(1/2+1/2*np.tanh(-1000.0*(GNDx-X_step+0.05)))
        print("OD")
    else:
        GNDy = 0.0*GNDx
    ...
    def plot_board(i,m,ax): #update function for animation
        ...
        #plot GRF's
        if (m.GRF[i,'BW'].value is not None) and (maxGRF!=0.0):
            magGRFBW = (m.GRF[i,'BW'].value+m.GRF_s[i,'BW'].value)/maxGRF
        else: 
            magGRFBW = 0
        if (m.GRF[i,'FW'].value is not None) and (maxGRF!=0.0):
            magGRFFW = (m.GRF[i,'FW'].value+m.GRF_s[i,'FW'].value)/maxGRF
        else: 
            magGRFFW = 0
...
def make_realtime_animation(m, N, hm, title, scenario, Y_step, X_step):
