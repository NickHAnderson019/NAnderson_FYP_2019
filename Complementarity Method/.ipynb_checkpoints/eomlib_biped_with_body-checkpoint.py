...
def get_EOMs_biped_with_body():
    g = sym.symbols('g')
    mb,mp,ml1_l,ml2_l,ml1_r,ml2_r = sym.symbols(['m_{body}','m_{pelvis}','m_{leg1_{left}}','m_{leg2_{left}}',
                                                 'm_{leg1_{right}}','m_{leg2_{right}}']) # mass

    lb,lp,ll1_l,ll2_l,ll1_r,ll2_r = sym.symbols(['l_{body}','l_{pelvis}','l_{leg1_{left}}','l_{leg2_{left}}',
                                                 'l_{leg1_{right}}','l_{leg2_{right}}']) # length

    Inb,Inp,Inl1_l,Inl2_l,Inl1_r,Inl2_r = sym.symbols(['I_{body}','I_{pelvis}','I_{leg1_{left}}','I_{leg2_{left}}',
                                                       'I_{leg1_{right}}','I_{leg2_{right}}']) # moment of intertia

    # generalized coordinates
    xp,yp,thp,thb,thl_l,thl_r,r_l,r_r = sym.symbols(['x_{pelvis}','y_{pelvis}','\\theta_{pelvis}','\\theta_{body}',
                                                                 '\\theta_{leg_{left}}','\\theta_{leg_{right}}',
                                                                 'r_{left}','r_{right}']) 

    dxp,dyp,dthp,dthb,dthl_l,dthl_r,dr_l,dr_r = sym.symbols(['\dot{x}_{pelvis}','\dot{y}_{pelvis}','\dot{\\theta}_{pelvis}',                                               '\dot{\\theta}_{body}','\dot{\\theta}_{leg_{left}}','\dot{\\theta}_{leg_{right}}',
                                                             '\dot{r}_{left}','\dot{r}_{right}']) 

    ddxp,ddyp,ddthp,ddthb,ddthl_l,ddthl_r,ddr_l,ddr_r = sym.symbols(['\ddot{x}_{pelvis}','\ddot{y}_{pelvis}',
                                                                     '\ddot{\\theta}_{pelvis}','\ddot{\\theta}_{body}',
    ...
    F_a_l,F_a_r,GRFx_l,GRFy_l,GRFx_r,GRFy_r = sym.symbols(['F_{left}','F_{right}',
                                                           'G_{x_{left}}','G_{y_{left}}',
                                                           'G_{x_{right}}','G_{y_{right}}']) 
    tau_b,tau_l_l,tau_l_r = sym.symbols(['\\tau_{body}','\\tau_{leg_{left}}','\\tau_{leg_{right}}'])

    # Unpickle stored EOMs
    infile = open('biped_with_body_EOMs','rb')
    data = pkl.load(infile)
    infile.close()
    ...
    sym_list = [g,mp,mb,ml1_l,ml2_l,ml1_r,ml2_r,
                lp,lb,ll1_l,ll2_l,ll1_r,ll2_r,
                Inp,Inb,Inl1_l,Inl2_l,Inl1_r,Inl2_r,
                xp,yp,thp,thb,thl_l,r_l,thl_r,r_r,
                dxp,dyp,dthp,dthb,dthl_l,dr_l,dthl_r,dr_r,
                ddxp,ddyp,ddthp,ddthb,ddthl_l,ddr_l,ddthl_r,ddr_r,
                F_a_l,F_a_r,tau_b,tau_l_l,tau_l_r,GRFx_l,GRFy_l,GRFx_r,GRFy_r]
    ...
    lambEOMthl_l_biped = sym.lambdify(sym_list,EOMs_biped[4],modules = [func_map])
    lambEOMthl_r_biped = sym.lambdify(sym_list,EOMs_biped[5],modules = [func_map])
    lambEOMr_l_biped   = sym.lambdify(sym_list,EOMs_biped[6],modules = [func_map])
    lambEOMr_r_biped   = sym.lambdify(sym_list,EOMs_biped[7],modules = [func_map])
    
    return [EOMs_biped,[lambEOMxp_biped, lambEOMyp_biped, lambEOMthp_biped, lambEOMthb_biped, lambEOMthl_l_biped, lambEOMr_l_biped, lambEOMthl_r_biped, lambEOMr_r_biped]]
